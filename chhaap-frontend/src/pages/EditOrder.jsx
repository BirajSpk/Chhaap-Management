...
import { Plus, Trash2, X, ArrowLeft, FileText, Eye, EyeOff } from 'lucide-react'
  ...
  return (
    <div className="p-4 md:p-6">
      <div className="flex items-center gap-3 mb-6">
        <Link to={`/orders/${id}`} className="p-2 text-slate-500 hover:bg-slate-100 rounded-lg">
      ...
      <form onSubmit={handleSubmit} className="space-y-6 max-w-3xl">
        <div className="bg-white rounded-xl border border-slate-200 p-4 md:p-5 space-y-3">
          ...
          <div className="grid grid-cols-1 sm:grid-cols-2 gap-3">
            ...
            <div>
              ...
              <input type="tel" inputMode="tel" maxLength={20} pattern="[0-9+\-\s()]*"
                value={customerPhone} onChange={e => setCustomerPhone(sanitizePhone(e.target.value))}
        ...
        <div className="bg-white rounded-xl border border-slate-200 p-4 md:p-5 space-y-3">
          <div className="flex items-center justify-between">
            ...
            <div className="flex gap-2">
              <button type="button" onClick={addCustomItem} className="flex items-center gap-1 text-sm text-purple-600 hover:text-purple-700 font-medium">
                <FileText className="w-4 h-4" /> <span className="hidden sm:inline">Custom</span>
              </button>
          ...
          {items.map((item, idx) => (
            <div key={idx} className="flex flex-col sm:flex-row gap-2 p-3 bg-slate-50 rounded-lg">
              ...
              <div className="flex gap-2 items-center sm:items-start">
                ...
                <button type="button" onClick={() => removeItem(idx)} className="mt-4 p-2 text-red-500 hover:bg-red-50 rounded shrink-0">
        ...
        <div className="fixed inset-0 bg-black/40 flex items-center justify-center z-50" onClick={() => setShowConfirm(false)}>
          <div className="bg-white rounded-xl p-4 md:p-6 w-full max-w-lg mx-4 max-h-[90vh] overflow-y-auto" onClick={e => e.stopPropagation()}>
            ...
            <p className="text-sm text-slate-600 mb-4 bg-amber-50 border border-amber-200 rounded-lg px-3 py-2">
              Please confirm the updated order details with the customer before proceeding.
            </p>
            ...
            <div className="text-right text-lg font-bold text-slate-800 mt-4 mb-4">Total: ₹{total.toLocaleString()}</div>
