...
import { Plus, Trash2, X, FileText, Eye, EyeOff, ArrowLeft, UserPlus, Ruler, Camera, MessageCircle, Music2, Phone, Store, Globe, Wifi, Banknote } from 'lucide-react'
...
export default function NewOrder() {
  ...
  const [items, setItems] = useState([{ product_id: '', quantity: 1, sold_price: '', cost_price: '', is_dimension: false, length: '', breadth: '', unit: 'inches' }])
  ...
  const handleProductSelectDim = (idx, product) => {
    const newItems = [...items]
    if (product.is_dimension_product) {
      newItems[idx] = { product_id: product.id, quantity: 1, sold_price: 0, cost_price: 0, is_dimension: true, length: '', breadth: '', unit: 'inches' }
    }
    setItems(newItems)
    setProductSearch({ ...productSearch, [idx]: '' })
  }
  ...
  const validate = () => {
    ...
    if (paymentMethod === 'Hybrid') {
      const onAmt = Number(onlineAmount || 0)
  ...
  return (
    <div className="p-4 md:p-6">
      <div className="flex items-center gap-3 mb-6">
        <Link to="/orders" className="p-2 text-slate-500 hover:bg-slate-100 rounded-lg"><ArrowLeft className="w-5 h-5" /></Link>
        <h1 className="text-xl md:text-2xl font-bold text-slate-800">New Order</h1>
      </div>
      ...
      <form onSubmit={handleSubmit} className="space-y-6 max-w-3xl">
          <div className="bg-white rounded-xl border border-slate-200 p-4 md:p-5 space-y-3">
            ...
            <div><label className="block text-sm font-medium text-slate-600 mb-1">Address *</label>
              <textarea value={customerAddress} onChange={e => setCustomerAddress(e.target.value)} rows={2}
                className="w-full px-3 py-2 border border-slate-300 rounded-lg text-sm focus:outline-none focus:ring-2 focus:ring-cyan-500 resize-none" required /></div>
          ...
          <div>
            ...
            <div className="flex flex-wrap gap-1.5">
              {SOURCES.map(s => {
                ...
                const isActive = orderSource === s
                return (
                  <button key={s} type="button" onClick={() => setOrderSource(isActive ? '' : s)}
                    className={`flex items-center gap-1.5 px-2.5 py-1.5 rounded-lg text-xs font-medium border transition-colors ${
                      isActive ? 'bg-cyan-50 border-cyan-300 text-cyan-700' : 'bg-white border-slate-300 text-slate-600 hover:bg-slate-50'
                    }`}>
                    {Icon && <Icon className="w-3.5 h-3.5" />}
                    {s === 'Phone Call' ? 'Phone' : s}
                  </button>
                )
              })}
            </div>
        ...
        <button type="submit" className="w-full py-3 bg-cyan-600 text-white rounded-lg hover:bg-cyan-700 font-medium">Create Order</button>
      </form>
          ...
          <div className="bg-white rounded-xl p-4 md:p-6 w-full max-w-sm mx-4" onClick={e => e.stopPropagation()}>
            ...
            <div className="space-y-3">
              ...
              <div className="grid grid-cols-1 sm:grid-cols-2 gap-2">
                <div><label className="block text-sm font-medium text-slate-600 mb-1">Cost Price</label>
                  <input type="number" min="0" value={quickForm.cost_price} onChange={e => setQuickForm({ ...quickForm, cost_price: e.target.value })}
                    className="w-full px-3 py-2 border border-slate-300 rounded-lg text-sm focus:outline-none focus:ring-2 focus:ring-cyan-500" /></div>
                <div><label className="block text-sm font-medium text-slate-600 mb-1">Selling Price</label>
      ...
      {showConfirm && (
        <div className="fixed inset-0 bg-black/40 flex items-center justify-center z-50" onClick={() => setShowConfirm(false)}>
          <div className="bg-white rounded-xl p-4 md:p-6 w-full max-w-lg mx-4 max-h-[90vh] overflow-y-auto" onClick={e => e.stopPropagation()}>
            ...
            <p className="text-sm text-slate-600 mb-4 bg-amber-50 border border-amber-200 rounded-lg px-3 py-2">Please confirm the order details with the customer before proceeding.</p>
                  ...
                  {items.filter(i => i.is_custom || i.product_id).map((item, idx) => {
                    ...
                    return (
                      <tr key={idx} className="border-b border-slate-100">
                        ...
                        <td className="py-2 text-right font-medium text-slate-700">रु{(Number(item.sold_price) * Number(item.quantity)).toLocaleString()}</td>
                      </tr>
                    )
                  })}
                </tbody>
            ...
            <div className="text-right text-lg font-bold text-slate-800 mt-4 mb-4">Total: रु{total.toLocaleString()}</div>
            <div className="flex gap-3">
              <button onClick={handleConfirm} className="flex-1 py-2.5 bg-cyan-600 text-white rounded-lg hover:bg-cyan-700 font-medium text-sm">Confirm & Create</button>
